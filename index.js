		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: 'Este comando só pode ser usado quando eu for adm'
				}
			}
			...
			switch(command) {
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Só funciona com fotos macaco')
					}
					break
				case 'stiker':
				case 'sticker':
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('Oi papai, para que eu possa xlonar a foto de perfil de alguem você deve primeiro marcar a pessoa')
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Não encontrei essa pessoa aqui')
					...
					try {
						...
						mentions(`Pronto papai 🥰, clonei a foto desse pau no cu aqui : @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Falha')
					}
					break
